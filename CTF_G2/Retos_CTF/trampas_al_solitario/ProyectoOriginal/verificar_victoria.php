...
if (isset($input['puntuacion']) && isset($input['token'])) {
    ...
    if ($puntuacion_cliente >= 5000) {
        ...
        if (hash_equals($token_esperado, $token_cliente)) {
            // ¡Éxito! Devolver la flag.
            $response = ['exito' => true, 'flag' => $flag];
        } else {
            $response['mensaje'] = 'Token inválido.';
            // Log opcional para depuración del CTF por si algo falla:
            // error_log("Verificación fallida. Recibido: $token_cliente, Esperado: $token_esperado, Puntuación: $puntuacion_cliente");
        }
    } else {
