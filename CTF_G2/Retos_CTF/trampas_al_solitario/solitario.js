...
var puntos = 0;
var secreto = "SolitarioWeb2025"; // Secreto para la verificación
...
function agregarPilaAlta(div, diviCarta, num){
	...
	desSeleccionar();
	 puntos = puntos+5;
	   debugger; // Pausa aquí para modificar 'puntos' en la consola
	 verificarPuntuacion(); // Se ejecuta al reanudar (F8)
...
function verificarPuntuacion() {
	   var dataToSend = {
	       puntuacion: puntos,
	       token: btoa(puntos + ':' + secreto) // btoa es el equivalente en JS a base64_encode en PHP
	   };

	   $.ajax({
	       url: 'verificar_victoria.php',
	       type: 'POST',
	       contentType: 'application/json', // Especificar que enviamos JSON
	       data: JSON.stringify(dataToSend), // Convertir el objeto a string JSON
	       dataType: 'json', // Esperamos una respuesta JSON
	       success: function (response) {
	           console.log("Respuesta de verificación:", response); // Log para depuración
	           if (response.exito === true && response.flag) {
	               alert("¡Felicidades! Has obtenido la flag:\n\n" + response.flag);
	               // Opcional: Detener el juego o realizar otra acción
	               DetenerCrono();
	           }
	           // No hacemos nada si exito es false, la verificación se hará en la siguiente actualización
	       },
	       error: function (jqXHR, textStatus, errorThrown) {
	           console.error("Error al verificar puntuación:", textStatus, errorThrown);
	           // Opcional: Mostrar un mensaje de error al usuario
	           // alert("Error al contactar con el servidor para verificar la puntuación.");
	       }
	   });
}
